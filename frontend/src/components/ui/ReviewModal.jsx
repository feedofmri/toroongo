...
export default function ReviewModal({ isOpen, onClose, product, orderId, onReviewSubmitted }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || t('reviews.errorSubmitting', 'Failed to submit review.'));
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-border-soft overflow-hidden flex flex-col">
                ...
                <div className="px-6 py-4 border-b border-border-soft flex items-center justify-between bg-surface-bg">
                    <h3 className="font-bold text-text-primary">{t('reviews.writeReview', 'Write a Review')}</h3>
                    <button 
                        onClick={onClose}
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center gap-4 p-4 bg-surface-bg rounded-xl border border-border-soft mb-6">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-text-primary line-clamp-1">{product.title}</p>
                            <p className="text-xs text-text-muted mt-0.5">{t('reviews.rateYourExperience', 'How would you rate your experience?')}</p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder={t('reviews.commentPlaceholder', 'Tell others what you think about this product...')}
