...
import { X, Plus, Loader, Image, Video, Trash2, GripVertical, Link2 } from 'lucide-react';
        ...
        <div className="group relative rounded-xl overflow-hidden border border-border-soft bg-surface-bg aspect-square">
            ...
            <div className="absolute top-2 left-2">
                <span className={`flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full backdrop-blur-sm
                    ...
                    {isVideo ? <><Video size={10} /> Video</> : <><Image size={10} /> Image</>}
                </span>
            </div>
        ...
        if (isOpen) {
            ...
            if (editProduct) {
                setFormData({
                    ...
                    original_price: (editProduct.original_price || editProduct.originalPrice)?.toString() || '',
                    stock: editProduct.stock?.toString() || '',
                    category: editProduct.category || '',
                    meta_description: editProduct.meta_description || '',
                });
                setMediaUrls(editProduct.images || (editProduct.image_url ? [editProduct.image_url] : (editProduct.imageUrl ? [editProduct.imageUrl] : [])));
            } else {
                setFormData({
                    title: '',
                    description: '',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-border-soft">
                    <h2 className="text-xl font-bold text-text-primary">{isEditMode ? 'Edit Product' : 'Add New Product'}</h2>
                    <button
                        onClick={onClose}
                    ...
                    <form id="productForm" onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex justify-end mt-1">
                                <span className={`text-[10px] ${formData.meta_description?.length > 150 ? 'text-amber-500' : 'text-text-muted'}`}>
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-muted mb-3">
                                Add image & video URLs. The first image will be used as the primary product image.
                                Supports direct image links, MP4 videos, and YouTube URLs.
                            </p>

                            {/* URL Input */}
                            <div className="flex gap-2 mb-3">
                                <div className="relative flex-1">
                                    <Link2 size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
                                    <input
                                        type="text"
                                        value={newMediaUrl}
                                        onChange={(e) => { setNewMediaUrl(e.target.value); setUrlError(''); }}
                            ...
                            {mediaUrls.length > 0 ? (
                                ...
                                <div className="border-2 border-dashed border-border-soft rounded-xl p-8 text-center">
                                    ...
                                    <p className="text-xs text-text-muted/70 mt-1">Paste image or video URLs above to add product media</p>
                                </div>
                            )}
                        </div>
                    </form>
