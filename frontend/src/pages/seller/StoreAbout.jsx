    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="max-w-3xl">
                <h2 className="text-2xl font-bold text-text-primary mb-6">About {seller.store_name || seller.name}</h2>
                ...
                {seller.seller_settings?.about_content ? (
                    ...
                    <p className="text-text-muted leading-relaxed mb-8">
                        {seller.description || (seller.store_name || seller.name) + ' is committed to providing our customers with the highest quality products and exceptional service.'} Our team carefully curates every item in our catalog to ensure it meets
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
                    {[
                        { icon: Star, label: 'Rating', value: Number(seller.rating || 0).toFixed(1) },
                        { icon: Package, label: 'Products', value: seller.total_products || sellerProducts.length },
                        { icon: Calendar, label: 'Member Since', value: new Date(seller.joined_date || seller.created_at || new Date().toISOString()).getFullYear() },
                        { icon: MapPin, label: 'Location', value: seller.location || 'United States' },
                    ].map((stat) => (
                ...
                <div className="space-y-4 mb-8">
                    {(seller.seller_settings?.benefits || [
