...
export default function ContactPage() {
    ...
    return (
        <div className="animate-fade-in max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <h1 className="text-3xl font-bold text-text-primary mb-3 text-center">{t('contact.title')}</h1>
            <p className="text-text-muted text-center mb-12">{t('contact.subtitle')}</p>

            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                {/* Contact info */}
                <div className="lg:col-span-2 space-y-6">
                    {[
                        { icon: Mail, label: t('contact.email'), value: 'support@toroongo.com' },
                        { icon: Phone, label: t('contact.phone'), value: '+1 (800) 123-4567' },
                        { icon: MapPin, label: t('contact.address'), value: '100 Market St, Suite 400\nSan Francisco, CA 94105' },
                    ].map((item) => (
                ...
                <form onSubmit={(e) => e.preventDefault()} className="lg:col-span-3 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input type="text" placeholder={t('contact.firstName')} className={inputClass} />
                        <input type="text" placeholder={t('contact.lastName')} className={inputClass} />
                    </div>
                    <input type="email" placeholder={t('contact.emailLabel')} className={inputClass} />
                    <input type="text" placeholder={t('contact.subject')} className={inputClass} />
                    <textarea rows={5} placeholder={t('contact.message')} className={`${inputClass} resize-none`} />
