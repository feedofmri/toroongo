    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-text-muted">
          {t("common.loading", "Loading your reviews...")}
        </p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl border border-border-soft overflow-hidden">
        {reviews.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <Link
              to="/account/orders"
              className="px-6 py-2.5 bg-brand-primary text-white text-sm font-semibold rounded-xl hover:bg-brand-secondary transition-colors inline-block"
            >
          ...
          <div className="divide-y divide-border-soft">
            {reviews.map((review) => (
              <div
                key={review.id}
                className="p-6 hover:bg-surface-bg/50 transition-colors"
              >
                <div className="flex flex-col sm:flex-row items-start gap-4">
                  <Link
                    to={`/product/${review.product?.slug || review.product_id}`}
                    className="flex-shrink-0"
                  >
                    <img
                      src={
                        review.product?.image_url || review.product?.imageUrl
                      }
                  ...
                  <div className="flex-1 min-w-0 w-full">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <Link
                          to={`/product/${review.product?.slug || review.product_id}`}
                          className="text-sm font-bold text-text-primary hover:text-brand-primary transition-colors line-clamp-1"
                        >
                    ...
                    <div className="mt-3 text-sm text-text-primary bg-surface-bg p-4 rounded-xl border border-border-soft italic relative">
                      <MessageSquare
                        size={14}
                        className="absolute -top-2 -left-2 text-brand-primary bg-white rounded-full p-1 border border-border-soft shadow-sm"
                      />
                      "{review.comment}"
                    </div>
                      ...
                      <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {review.media.map((url, idx) => {
                          ...
                          return (
                            <div
                              key={`${url}-${idx}`}
                              className="rounded-xl overflow-hidden border border-border-soft bg-white aspect-square"
                            >
                              {video ? (
                                embed ? (
                                  <iframe
                                    src={embed}
                                    title={`Review media ${idx + 1}`}
                    ...
                    <div className="mt-3 flex items-center justify-end">
                      ...
                      >
                        {t("reviews.viewProduct", "View Product")}{" "}
