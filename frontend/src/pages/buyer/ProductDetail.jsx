    ...
    <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2">
      {media.map((url, index) => {
        ...
        return (
          <div
            key={`${url}-${index}`}
            className="relative rounded-lg overflow-hidden border border-border-soft bg-white aspect-square"
          >
            {video ? (
              embed ? (
                <iframe
                  src={embed}
                  title={`Review media ${index + 1}`}
...
function MediaGallery({ product, selectedVariantValues }) {
  // Determine if a variation-specific image should be shown
  const variationImage = (() => {
    const groups = Array.isArray(product.variations) ? product.variations : [];
    for (const g of groups) {
      const sel = selectedVariantValues?.[g.name];
      if (!sel) continue;
      const found = (g.values || []).find((v) => {
        if (typeof v === "string") return v === sel;
        return v.value === sel || v.value == sel;
      });
      if (found) return found.image_url || found.imageUrl || null;
    }
    return null;
  })();

  const baseMedia = product.images?.length
    ? product.images
    : product.imageUrl
      ? [product.imageUrl]
      : [];

  const allMedia = variationImage
    ? [variationImage, ...baseMedia.filter((m) => m !== variationImage)]
    : baseMedia;

  const [selectedIndex, setSelectedIndex] = useState(0);
  const selectedMedia =
    allMedia[selectedIndex] || product.imageUrl || variationImage;
  ...
  React.useEffect(() => {
    // reset to first media when variation image changes
    setSelectedIndex(0);
  ...
  return (
    <div className="relative flex gap-3">
      ...
      {allMedia.length > 1 && (
        <div
          className="hidden sm:flex flex-col gap-2 w-16 flex-shrink-0 max-h-[500px] overflow-y-auto pr-1
      ...
      {product.badge && (
        <span
          className={`absolute top-4 ${allMedia.length > 1 ? "sm:left-[76px] left-4" : "left-4"} text-sm font-semibold px-3 py-1.5 rounded-full z-10
                    ${
                      product.badge === "Sale"
...
export default function ProductDetail() {
  ...
  const selectedVariant = variationGroups
    .map((variation) => {
      const value =
        selectedVariantValues[variation.name] || variation.values?.[0] || "";
      return { name: variation.name, value };
    })
  ...
  // Determine if any selected variation value has an override price
          ...
          <div className="flex flex-col">
                  ...
                  <div key={variation.name} className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(variation.values || []).map((value) => {
                        const isActive =
                          (selectedVariantValues[variation.name] ||
                          ...
                          <button
                            ...
                            className={`px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${
                              isActive
                                ...
                                : "border-border-soft text-text-primary hover:border-gray-300"
                            }`}
                          >
                            {value}
                          </button>
            ...
            {(() => {
              const sellerData = sellers.find(
                (s) => String(s.id) === String(product.sellerId),
              );
              return sellerData ? (
                <div className="bg-surface-bg rounded-xl p-4 mb-6 border border-border-soft">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <div className="flex items-center gap-0.5">
                          ...
                          <span className="text-xs font-semibold text-text-primary">
                            {Number(sellerData.rating || 0).toFixed(1)}
                          </span>
                        </div>
                  ...
                  >
                    <Store size={13} /> {t("product.visitShop")}{" "}
          ...
          {activeTab === "description" ? (
            <div className="max-w-3xl text-text-muted text-sm leading-relaxed space-y-6">
              ...
              {!product.specifications && !product.description && (
                <>
                  <h4 className="text-text-primary font-semibold pt-2">
            ...
            <div className="max-w-3xl space-y-6">
              {loadingReviews ? (
                ...
                reviews.map((review) => (
                  <div
                    key={review.id}
                    className="p-5 bg-surface-bg rounded-xl border border-border-soft"
                  >
