...
export default function Checkout() {
        ...
        cartSummary
          .map((item) => {
            const p = allProducts.find(
              (pr) => String(pr.id) === String(item.id),
            );
  ...
  useEffect(() => {
    const loadShippingAreas = async () => {
      if (!user || cartSellerIds.length === 0) {
        setShippingAreas([]);
        setSelectedShippingAreaId(null);
        return;
      }
  ...
  const shippingCost = shippingQuote.total;
  ...
  useEffect(() => {
    const shippingReady =
      ...
      shipping.address &&
      shipping.zip;
    if (!shippingReady || cartSummary.length === 0) {
  ...
  const handlePlaceOrder = async () => {
    ...
    let paymentDesc =
      "Credit Card ending in " + (payment.cardNumber || "4242").slice(-4);
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center animate-fade-in">
        ...
        <h2 className="text-2xl font-bold text-text-primary mb-2">
          {t("checkout.loginRequiredTitle", "Login Required")}
        </h2>
        <p className="text-text-muted mb-6">
          {t(
            "checkout.loginRequiredMsg",
            "Please log in to your account to continue securely with your checkout.",
          )}
        </p>
        <Link
          to="/login"
        ...
        <div className="flex items-center justify-center mb-10">
          {steps.map((s, idx) => (
            <React.Fragment key={s.num}>
              ...
              {idx < steps.length - 1 && (
                <div
                  className={`w-8 sm:w-16 h-px mx-2 ${step > s.num ? "bg-brand-primary" : "bg-border-soft"}`}
                />
              )}
            </React.Fragment>
          ))}
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <button
                        ...
                        className={`p-4 border text-left rounded-xl transition-all flex flex-col justify-center items-center gap-1 border-dashed ${
                          !selectedAddressId
                            ? "border-brand-primary bg-brand-primary/5 ring-1 ring-brand-primary"
                            : "border-border-soft hover:border-gray-300 bg-white"
                        }`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange(setShipping, "lastName", e.target.value)
                    }
                    className={inputClass}
                  />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange(setShipping, "email", e.target.value)
                    }
                    className={inputClass}
                  />
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange(setShipping, "phone", e.target.value)
                    }
                    className={inputClass}
                  />
                </div>
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange(setShipping, "address", e.target.value)
                  }
                  className={inputClass}
                />
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <input
                    ...
                    value={shipping.city}
                    onChange={(e) =>
                      handleInputChange(setShipping, "city", e.target.value)
                    }
                    className={inputClass}
                  />
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange(setShipping, "state", e.target.value)
                    }
                    className={inputClass}
                  />
                  <input
                    ...
                    value={shipping.zip}
                    onChange={(e) =>
                      handleInputChange(setShipping, "zip", e.target.value)
                    }
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-text-primary">
                      {t("checkout.shippingArea", "Shipping Area")}
                    </h3>
                    <span className="text-xs text-text-muted">
                      {shippingAreas.length} option
                      {shippingAreas.length === 1 ? "" : "s"}
                    </span>
                  </div>
                  <p className="text-xs text-text-muted">
                    {t(
                      "checkout.shippingAreaDesc",
                      "Select the seller-defined shipping area after entering your address. The selected area's fee is used in the total.",
                    )}
                  </p>
                  {shippingAreas.length === 0 ? (
                    <div className="p-4 rounded-xl border border-border-soft bg-surface-bg text-sm text-text-muted">
                      {t(
                        "checkout.noShippingAreas",
                        "No shipping areas are available for the items in your cart.",
                      )}
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {shippingAreas.map((area) => (
                        <button
                          key={area.id}
                          type="button"
                          onClick={() => setSelectedShippingAreaId(area.id)}
                ...
                <div className="mt-2 p-4 bg-surface-bg rounded-xl border border-border-soft">
                  <p className="text-xs text-text-muted">
                    {t(
                      "checkout.sellerShippingNote",
                      "Shipping costs are set by individual sellers and are included in your total.",
                    )}
                  </p>
                </div>
                ...
                <button
                  onClick={() => {
                    if (validateShipping() && selectedShippingAreaId)
                      setStep(2);
                  }}
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3 mb-8">
                  {[
                    {
                      id: "credit_card",
                      label: t("checkout.creditCard", "Credit Card"),
                      icon: CreditCard,
                    },
                    {
                      id: "cod",
                    ...
                    {
                      id: "bkash",
                      label: t("checkout.bkash", "bKash"),
                      icon: Lock,
                    },
                    {
                      id: "nagad",
                      label: t("checkout.nagad", "Nagad"),
                      icon: Lock,
                    },
                    {
                      id: "paypal",
                    ...
                    <label
                      key={method.id}
                      className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-colors ${paymentMethod === method.id ? "border-brand-primary bg-brand-primary/5 ring-1 ring-brand-primary" : "border-border-soft hover:border-gray-300"}`}
                    >
                      <input
                        type="radio"
                        name="paymentMethod"
                  ...
                  <div className="space-y-4 animate-fade-in">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <input
                        ...
                        value={payment.cvv}
                        onChange={(e) =>
                          handleInputChange(setPayment, "cvv", e.target.value)
                        }
                        className={inputClass}
                      />
                    </div>
                ...
                {(paymentMethod === "bkash" || paymentMethod === "nagad") && (
                  <div className="space-y-4 animate-fade-in">
                    <p className="text-sm text-text-muted">
                      {t(
                        "checkout.bkashNagadDesc",
                        "Please enter your {{method}} account number. We will send an OTP for verification.",
                        {
                ...
                {paymentMethod === "paypal" && (
                  <div className="p-4 bg-surface-bg rounded-xl text-sm text-text-muted border border-border-soft animate-fade-in">
                    {t(
                      "checkout.paypalDesc",
                      'You will be redirected to PayPal to complete your purchase securely after clicking "Review Order".',
                    )}
                  </div>
                )}
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 bg-surface-bg rounded-xl border border-border-soft">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-semibold text-text-primary flex items-center gap-2">
                      ...
                      {t("checkout.shippingTo", "Shipping to")}
                    </h4>
                    <button
                      onClick={() => setStep(1)}
                      className="text-xs text-brand-primary font-medium hover:text-brand-secondary"
                    >
                ...
                <div className="border border-border-soft rounded-xl divide-y divide-border-soft">
                  {cartSummary.map((item) => {
                    ...
                    if (!p) return null;
                    const variantLabel =
                      Array.isArray(item.variant) && item.variant.length
                        ? item.variant
                            .map(
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={shippingQuote.loading || !!shippingQuote.error}
                    className="flex-1 py-3.5 bg-brand-primary text-white font-semibold rounded-xl
                             hover:bg-brand-secondary transition-colors flex items-center justify-center gap-2 shadow-lg shadow-brand-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
              ...
              <div className="space-y-2.5 text-sm">
                <div className="flex justify-between">
                  <span className="text-text-muted">
                    {t("checkout.subtotal", "Subtotal")} ({cartSummary.length}{" "}
                ...
                {shippingQuote.error && (
                  <p className="text-xs text-red-600">{shippingQuote.error}</p>
                )}
              </div>
            </div>
          </div>
