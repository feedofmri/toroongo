...
export default function OrderHistory() {
  ...
  const getProductInfo = (productId) => {
    ...
    return (
      p || {
        title: t("product.unknownProduct"),
        imageUrl: "",
        sellerId: t("product.unknownSeller"),
      }
    );
  };
                ...
                <div className="px-5 py-3 bg-surface-bg border-b border-border-soft flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-4 text-sm">
                    <span className="font-semibold text-text-primary">
                      {t("orders.orderNo", {
                        id: String(order.id).split("-")[0].toUpperCase(),
                      })}
                    </span>
                      ...
                      <div key={idx} className="p-5 flex gap-4">
                        <div className="w-16 h-16 rounded-xl overflow-hidden bg-surface-bg flex-shrink-0 border border-border-soft">
                          {pInfo.imageUrl ? (
                            ...
                            <div className="w-full h-full bg-border-soft"></div>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-text-muted mt-0.5">
                            {t("orders.soldBy", { seller: pInfo.sellerId })} ·{" "}
          ...
          <div className="bg-surface-bg w-full max-w-md rounded-2xl p-6 shadow-xl border border-border-soft flex flex-col">
            ...
            >
              ...
              <option value="Found a better price elsewhere">
                {t("orders.reasons.betterPrice")}
              </option>
              <option value="Ordered by mistake">
                {t("orders.reasons.mistake")}
              </option>
              <option value="Expected delivery time is too long">
                {t("orders.reasons.longDelivery")}
              </option>
